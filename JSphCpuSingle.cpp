...
void JSphCpuSingle::RunCellDivide(bool updateperiodic){
  ...
  //-Initiates Divide.
  CellDivSingle->Divide(Npb,Np-Npb-NpbPer-NpfPer,NpbPer,NpfPer,BoundChanged,Dcellc,Codec,Idpc,Posc,Timers,RestartDivideFull);
  ...
  //Log->Printf("Np:%u Npb:%u NpbOk:%u",Np,Npb,NpbOk);
...
void JSphCpuSingle::Run(std::string appname,JSphCfgRun *cfg,JLog2 *log){
  ...
  FreePartsInit();
  if(RestartBegin){
    RestartDivideFull=true;
    const unsigned NpPrev=Np;
    LoadRestartParticles();
    Np=PartsLoaded->GetCount();
    Log->Printf("JSphCpuSingle:Run NpPrev=%u Np=%u",NpPrev,Np);
    ...
    memcpy(Posc,PartsLoaded->GetPos(),sizeof(tdouble3)*Np);
    memcpy(Idpc,PartsLoaded->GetIdp(),sizeof(unsigned)*Np);
    memcpy(Codec,Codec,sizeof(unsigned)*Np);
    memcpy(Velrhopc,PartsLoaded->GetVelRhop(),sizeof(tfloat4)*Np);
    ...
    if(InOut)InOutRestart(TimeStepIni,NpPrev,Np);
